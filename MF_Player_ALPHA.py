...
Added: repeat option (toggle with command `repeat`).
Save as /mnt/data/MF_Player_ALPHA.py
"""
...
def show_ui(player, files, current_index, repeat_flag):
    """
    Clear screen and show:
      - current state, time, volume
      - playlist with marker on current
      - simple help line (includes repeat status)
    """
    clear_screen()
    ...
    repeat_status = "ON" if repeat_flag else "OFF"
    print(f"State: {state}   Track: {current_index + 1}/{len(files)}   Repeat: {repeat_status}")
    ...
    print("-" * 60)
    print("Commands: play | pause | stop | next | prev | repeat | v N | v+ | v- | m | exit")
...
def main_loop(player, instance, files, folder, start_index=0):
    ...
    Main loop:
      ...
      - repeat flag lives here (function-scope)
      - outer loop plays tracks; inner loop handles commands + automatic checks
    """
    current = clamp_index(start_index, len(files))

    # Define repeat flag at top-level of this function so it is visible everywhere inside.
    repeat = False
    ...
    while running:
        ...
        while True:
            # Refresh UI so user sees updated time/volume (show repeat status too)
            show_ui(player, files, current, repeat)

            # --- AUTOMATIC REPEAT CHECK (robust) ---
            # Read state, position and length once per loop.
            try:
                state = player.get_state()        # enum
            except Exception:
                state = None

            try:
                pos = player.get_time() or 0      # milliseconds
                length = player.get_length() or 0 # milliseconds
            except Exception:
                pos = 0
                length = 0

            # Conditions that indicate the track finished:
            # 1) VLC explicit Ended state, OR
            # 2) VLC reported Stopped, OR
            # 3) position is very near length (fallback for flaky state reporting)
            ended_by_state = state in (vlc.State.Ended, vlc.State.Stopped)
            ended_by_time = (length > 0 and pos >= max(0, length - 500))  # within 0.5s of end

            if repeat and (ended_by_state or ended_by_time):
                # restart the same track
                play_track(player, instance, files[current])
                # go to next iteration (do not prompt input this iteration)
                continue
            ...
            # Toggle repeat flag. Auto-restart is handled by the automatic check
            elif cmd == "repeat":
                repeat = not repeat
                print("Repeat ON" if repeat else "Repeat OFF")
